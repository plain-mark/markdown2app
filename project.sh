...
current_version=$(grep -o "__version__ = version = '[0-9.]*'" src/markdown2app/_version.py | grep -o "[0-9.]*")
...
cat > src/markdown2app/_version.py << EOF
...
echo ""
git add src/markdown2app/_version.py setup.py "$changelog_file" increment_init.py increment_setup.py
